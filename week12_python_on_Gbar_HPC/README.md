# PiWE Week 12: Python on Gbar (HPC)

Welcome to week 12 of this course!
...
You should have the final project nearly finished in your group.

**Course evaluation**: if you haven't done it , please take
several minutes to do it here: https://evaluering.dtu.dk/  
...
* Check the notes in the folder [`notes_and_tips`](
    ...
    final project. Note that there is new note on 
    [**Working with File Paths in Python**](
    ../notes_and_tips/6_note_on_file_paths.md).
* Check the [`example package`](https://github.com/ju-feng-dk/FinalProjectExample) for organization of the package.
...
2. We will look into how to create *virtual environments*.
   ...
   Today's exercises will be based on locally installed
...
- connect; your shell should show something like this: 
  ...
  **you should avoid running tasks taking more than a few minutes here.**

### Other ways
If you prefer to access the cluster through GUI (graphic user
interface), you can use the [ThinLinc](https://www.cendio.com/) client ([instructions](https://www.gbar.dtu.dk/index.php/faq/43-thinlinc) here). After the log-in, ThinLinc provides you with a traditional desktop interface. You can use also VScode to edit code through this interface.

Accessing storage on cluster can consult: https://www.hpc.dtu.dk/?page_id=59  

For example, you can mount your space on Gbar cluster as a
network drive on your computer, for Windows/Mac user, the address is:
   ```
   Windows \\home.cc.dtu.dk\username
   MAC smb://home.cc.dtu.dk/username
   ```
Moving files to and from the cluster using ssh keys can consult: https://www.hpc.dtu.dk/?page_id=4377
...
1. On our HPC system the default Python 3 version is 3.9.5.
   This is too old!
...
2. Once you have loaded the desired Python version, we can create
   ...
   to create it with, in this case 3.11.13.
...
- `run_turbie_parallel.py` is a code to run `turbie`
  ...
  You can replace [your_node] with your [hostname] in the original 
  terminal, which shows when you run `hostname`,
